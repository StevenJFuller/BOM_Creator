...
def create_spreadhseet(): # sets up MTO spreadsheet with drawing names (typically ISO numbers) along the top row.
    ...
    pdf_files = []
    sep_dwg_names = listdir('drawings_separated')
...
def convert_to_csv(): # converts seperated drawings' BOM's to csv format --note: 'area' may need to be manipulated using tabula-java to define BOM list on drawing
    ...
    tabula.convert_into_by_batch("drawings_separated", output_format="csv", lattice=True, area=(57.88,1154.442,788.22,1674.309))
    print("Done.")
...
def populate_spreadsheet():
    ...
    for i in range(len(csv_files)):
        print("Reading file {} of {}".format(i + 1, len(csv_files)))
        # populate data
        with open('drawings_separated/' + csv_files[i], 'r') as file:
